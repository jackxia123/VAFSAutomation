...
{
    [DataContract]
    public class ReadSignalsTimeout:MeasurementPoint
    {
        private string _msgName;
        private int _startTime;

        public ReadSignalsTimeout()
        {
            
        }

        public ReadSignalsTimeout(string msgName, int startTime = 0)
        {            
            MsgName = msgName;
            StartTime = startTime;
        ...
        public override DependentParameter EvalParameter
        {
            get
            {
                if (StartTime == 0)
                {
                    return new ScalarDependentParameter("read_signals_timeout", @"The values of the CAN signals are recorded when the message specified by this pa-rameter times out.
...
", MsgName, "");
                ...
                else
                {
                    ...
                    return new HashDependentParameter("read_signals_timeout", @"The values of the CAN signals are recorded when the message specified by this pa-rameter times out.
e.g. 1. read_signals_timeout       = 'ECT1S07'
2. read_signals_timeout   =  %('Message' => 'ENG1S01', 'Start_Time' => 17000)
#Message name must be same as the name defined in CAN mapping file.
